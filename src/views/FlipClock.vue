...
async function flip(oldValue, newValue) {
  // 找到原始值的卡片
  const orignalCard = document.getElementById(`card-bottom-${oldValue}`)
  // 设置翻转效果
  ...
  // 找到目标值的卡片
  const targetCard = document.getElementById(`card-top-${newValue}`)
  await delay(300)
...
<template>
  <div class="h-700px flex items-center justify-center bg-#000 box">
      ...
      <div class="w-210px h-210px flex gap-10px hour">
        <div class="w-100px h-full flex flex-col gap-10px hour-fronted">
          <div class="relative h-100px">
            <div v-for="i in 10" :id="`card-top-${i - 1}`" :key="i" class="absolute w-100px h-100px bg-#211f1f text-#41fe7e text-150px card-item card-item-top" :style="{ zIndex: 10 - i }">
              <span class="absolute top">{{ i - 1 }}</span>
            </div>
          </div>
          <div class="relative h-100px">
            <div v-for="i in 10" :id="`card-bottom-${i - 1}`" :key="i" class="absolute w-100px h-100px bg-#211f1f text-#41fe7e text-150px card-item card-item-bottom" :style="{ zIndex: 10 - i }">
